...
words = [word.lower() for word in open('french_names.txt', 'r', encoding="utf-8").read().splitlines()]
...
chars = sorted(list(set(''.join(words)) - set("ABCDEFGHIJKLMNOPQRSTUVWXYZ/")))
...
with open("french_names.txt", "r", encoding="utf-8") as f:
...
with open("cleaned_french_names_in_german_model.txt", "w", encoding="utf-8") as f:
...
for i in range(C.shape[0]):
    if i not in itos:  # Skip indices not in the vocabulary
        continue
...
with open("generated_french_names_in_german_model.txt", "w", encoding="utf-8") as f:
    for _ in range(20):
        out = []
